...
public class MapViewTrackingUtilities implements OsmAndLocationListener, IMapLocationListener,
    ...
    //AP - Turn off false
    //Also, The default mode will "center" the map at the pointer's current
...
//                if(resetRatios) {
//                    setLocationWithoutAnimation(location);
//                }
                //End AP
                if (location != null) {
    ...
    //AP
    private void setLocationWithoutAnimation(Location location) {
        if (!isMapLinkedToLocation) {
            setMapLinkedToLocation(true);
        }
        if (isMapLinkedToLocation && myLocation != null
                && mapView != null && !mapView.isUserMapInteractionActive()) {
            app.runInUIThread(() -> updateLocation(location));
        }
    }
